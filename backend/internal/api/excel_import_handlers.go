...
func (s *Server) previewExcelImport(w http.ResponseWriter, r *http.Request) {
	data, timezone, ok := readExcelUpload(w, r)
	if !ok {
		return
	}
	result, err := s.Services.ExcelImports.Preview(data, timezone, r.FormValue("employeeQuery"))
	if err != nil {
		writeError(w, err)
		return
	}
...
func readExcelUpload(w http.ResponseWriter, r *http.Request) ([]byte, string, bool) {
